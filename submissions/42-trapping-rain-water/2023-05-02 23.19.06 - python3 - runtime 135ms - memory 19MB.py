class Solution:
    def half_trap(self, half_height: List[int]) -> int:
        left_penultimates = [-1] * len(half_height)
        left_highest = [0] * len(half_height)
        for i in range(1, len(half_height)):
            if half_height[i] >= half_height[left_highest[i - 1]]:
                left_highest[i] = i
                left_penultimates[i] = left_highest[i - 1]
            else:
                left_highest[i] = left_highest[i - 1]
        p = len(half_height) - 1
        s = 0
        while p != -1:
            for h in half_height[left_penultimates[p] + 1 : p]:
                s += half_height[left_penultimates[p]] - h
            p = left_penultimates[p]
        return s
